...
\usepackage{epsfig,graphicx,parskip,setspace,tabularx,xspace,url,holtexbasic,alltt,proof,epstopdf}
...
\label{lemma:conflictingRegistersAppearInEachOthersConflicts}
\begin{alltt}\small
...
To prove this, we induct on the graph, and do a case split on whether the current vertex is equal to the one being updated. If it is, we use the fact that the new colour will not have been used before and the current vertex is not in its own conflicts list to show that it is satisfactory on the current vertex, and the inductive hypothesis to prove the remainder of the goal. If it is not, we use the fact that the current vertex does not belong to its list of conflicts to show that its colour cannot conflict with anything in the set, and we again use the inductive hypothesis to prove the rest.
...
\label{lemma:respectingRegisterConflictsRespectsConflictingSets}
\begin{alltt}\small
	\HOLTokenTurnstile{} \HOLConst{duplicate_free} \HOLFreeVar{live} \HOLTokenImp{}
...
Thus we have verified that the antecedent of the overall goal implies the antecedent of the simplified lemma on page~\pageref{lemma:respectingRegisterConflictsRespectsConflictingSets}. The full proof can now be derived using these lemmas, expanding the definition of \texttt{colouring\_ok\_alt} to make the goal match up to the consequent of the simplified lemma.
...
\begin{itemize}
	\item The colouring $c$ satisfies \texttt{colouring\_ok}. As has been shown, this holds for any colouring generated by the algorithm
...
The implemented colouring algorithms are good, but more intelligent colouring algorithms are available. For instance, the iterated register coalescing algorithm described in George \cite{irc} would be a good candidate. Again, this wouldn't affect many other things, but would potentially require some re-structuring of the graph colouring proofs.
...
This project has demonstrated the verification of a register allocator in a modular, extensible way. Although the resulting algorithm lacks some features of more advanced register allocators, it works effectively on the graphs tested and has proven behaviour. Chapter~\ref{sec:extensions} demonstrates how it can be extended in various ways without much extra work, and as discussed above the performance of critical functions could be improved relatively easily.

In general, formal verification of software programs is not currently seen as necessary, or indeed particularly useful -- automated unit and integration testing is less time-consuming and can usually cover the majority of cases. However, for more critical applications such as compilers, the benefits of verification are clear: rare bugs which would be difficult to track down are eliminated, complex optimisations can be implemented without worrying about safety, and the correctness proof provides a precise contract describing the system's behaviour. While formal verification is not always worthwhile, it is certainly the way forward for critical pieces of code and is beginning to gain popularity in such applications.
