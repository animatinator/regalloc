...
vertices which have already been coloured (Al-Omari \& Sabri \cite{degreeHeuristics}).
...
\begin{alltt}\small
	\infer{\HOLinline{\HOLConst{MEM} \HOLFreeVar{x} (\HOLConst{insert} \HOLFreeVar{y} \HOLFreeVar{xs})}}{\HOLinline{\HOLConst{MEM} \HOLFreeVar{x} \HOLFreeVar{xs}}}
\end{alltt}
...
\begin{alltt}\small
	\infer{\HOLinline{\HOLFreeVar{a} \HOLTokenNotIn{} \HOLConst{set} \HOLFreeVar{xs}}}{\HOLinline{\HOLFreeVar{a} \HOLTokenNotIn{} \HOLConst{set} (\HOLConst{insert} \HOLFreeVar{x} \HOLFreeVar{xs})}}
\end{alltt}
...
\begin{alltt}\small
	\infer{\HOLinline{\HOLConst{MEM} \HOLFreeVar{x} (\HOLConst{list_union} \HOLFreeVar{list} \HOLFreeVar{list\sp{\prime}})}}{\HOLinline{\HOLConst{MEM} \HOLFreeVar{x} \HOLFreeVar{list} \HOLTokenDisj{} \HOLConst{MEM} \HOLFreeVar{x} \HOLFreeVar{list\sp{\prime}}}}
\end{alltt}
...
To demonstrate that this heuristic improves generated colourings, the lowest-first colouring algorithm was run on the sample code from Chapter~\ref{sec:background} again, this time using the highest-degree-first heuristic. The resulting function is as follows:
...
We proved earlier on that the function \texttt{get\_registers} generates duplicate-free results (see Section~\ref{sec:constructingClashGraph}). Therefore, if the function mapping registers to edge lists preserves duplicate-freeness then the result is duplicate-free. A statement of this lemma is given below:
...
\begin{alltt}\small
	\HOLTokenTurnstile{} (\HOLTokenForall{}\HOLBoundVar{x}. \HOLFreeVar{f} \HOLBoundVar{x} = \HOLFreeVar{g} \HOLBoundVar{x}) \HOLTokenImp{} (\HOLFreeVar{f} = \HOLFreeVar{g})
\end{alltt}
